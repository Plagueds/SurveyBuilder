...
// ----- START OF MODIFIED FILE (v1.4.5 - Debugging Status Init) -----
import React, { useState, useEffect, useCallback } from 'react';

// ... (SETTING_CATEGORIES and styles remain the same) ...
const SETTING_CATEGORIES = { GENERAL: 'General Survey Status', COMPLETION: 'Survey Completion & Endings', ACCESS_SECURITY: 'Access & Security', BEHAVIOR_NAVIGATION: 'Behavior & Navigation', CUSTOM_VARIABLES: 'Custom Variables', APPEARANCE: 'Appearance & Branding', };
...
const SurveySettingsPanel = ({ isOpen, onClose, initialSurveyData, onSave, surveyId }) => {
    ...
    const [surveyStatus, setSurveyStatus] = useState('draft'); // Initialized to 'draft'
    ...
    const mergeWithDefaults = useCallback((settingsFromProps) => {
        // ... (mergeWithDefaults logic is the same as v1.4.4, not re-pasting for brevity) ...
        const defaults = { completion: { type: 'thankYouPage', thankYouMessage: 'Thank you for completing the survey!', showResponseSummary: false, showScore: false, redirectUrl: '', passResponseDataToRedirect: false, disqualificationType: 'message', disqualificationMessage: 'Unfortunately, you do not qualify to continue with this survey.', disqualificationRedirectUrl: '', surveyClosedMessage: 'This survey is currently closed. Thank you for your interest.', }, accessSecurity: { linkExpirationDate: null, maxResponses: 0, passwordProtectionEnabled: false, surveyPassword: '', }, behaviorNavigation: { autoAdvance: false, questionNumberingEnabled: true, questionNumberingFormat: '123', saveAndContinueEnabled: false, saveAndContinueEmailLinkExpiryDays: 7, saveAndContinueMethod: 'email', }, customVariables: [], appearance: {}, }; const merged = {}; for (const categoryKey in defaults) { if (defaults.hasOwnProperty(categoryKey)) { merged[categoryKey] = { ...defaults[categoryKey], ...(settingsFromProps?.[categoryKey] || {}) }; if (categoryKey === 'customVariables' && !Array.isArray(merged[categoryKey])) { merged[categoryKey] = settingsFromProps?.[categoryKey] ? [...settingsFromProps[categoryKey]] : []; } if (categoryKey === 'behaviorNavigation') { if (typeof merged[categoryKey] !== 'object' || merged[categoryKey] === null) { merged[categoryKey] = { ...defaults[categoryKey] }; } const bn = merged[categoryKey]; const validMethods = ['email', 'code', 'both']; if (!validMethods.includes(bn.saveAndContinueMethod)) { bn.saveAndContinueMethod = defaults.behaviorNavigation.saveAndContinueMethod; } let days = bn.saveAndContinueEmailLinkExpiryDays; if (typeof days !== 'number' || isNaN(days) || days < 1) { days = defaults.behaviorNavigation.saveAndContinueEmailLinkExpiryDays; } else if (days > 90) { days = 90; } bn.saveAndContinueEmailLinkExpiryDays = days; } } } for (const key in settingsFromProps) { if (settingsFromProps.hasOwnProperty(key) && !defaults.hasOwnProperty(key)) { merged[key] = settingsFromProps[key]; } } return merged;
    }, []);

    useEffect(() => {
        // <<< ADD LOGGING HERE >>>
        console.log('[SurveySettingsPanel useEffect] isOpen:', isOpen);
        console.log('[SurveySettingsPanel useEffect] initialSurveyData received:', JSON.stringify(initialSurveyData, null, 2));
        ...
        if (isOpen) {
            try {
                ...
                const statusFromProps = initialSurveyData?.status;
                console.log('[SurveySettingsPanel useEffect] Status from props (initialSurveyData.status):', statusFromProps);
                
                const newStatusToSet = statusFromProps || 'draft';
                setSurveyStatus(newStatusToSet);
                console.log('[SurveySettingsPanel useEffect] surveyStatus state set to:', newStatusToSet);

            } catch (error) {
                console.error("Error merging settings in SurveySettingsPanel:", error);
                setCurrentSettings(mergeWithDefaults({}));
                setSurveyStatus('draft'); // Fallback
                console.log('[SurveySettingsPanel useEffect] surveyStatus state set to draft due to error.');
            }
        }
    }, [isOpen, initialSurveyData, mergeWithDefaults]);

    // Add another useEffect to log surveyStatus when it changes
    useEffect(() => {
        console.log('[SurveySettingsPanel surveyStatus changed] New surveyStatus value:', surveyStatus);
    }, [surveyStatus]);


    if (!isOpen || Object.keys(currentSettings).length === 0) {
        // console.log('[SurveySettingsPanel] Not rendering because not open or currentSettings empty.');
        return null;
    }
    // console.log('[SurveySettingsPanel] Rendering. Current surveyStatus for dropdown:', surveyStatus);

    const handleNestedChange = (category, field, value) => { /* ... same as v1.4.4 ... */ setCurrentSettings(prev => ({ ...prev, [category]: { ...(prev[category] || {}), [field]: value } })); };
    const handleInputChange = (category, e) => { /* ... same as v1.4.4 ... */ const { name, value, type, checked } = e.target; let valToSet; if (type === 'checkbox') { valToSet = checked; } else if (type === 'number') { if (name === 'saveAndContinueEmailLinkExpiryDays') { valToSet = value === '' ? '' : parseInt(value, 10); } else { const numVal = parseInt(value, 10); valToSet = isNaN(numVal) ? 0 : numVal; } } else { valToSet = value; } handleNestedChange(category, name, valToSet); };
    const getValidatedExpiryDays = (currentDaysValue) => { /* ... same as v1.4.4 ... */ let days = parseInt(currentDaysValue, 10); if (isNaN(days) || days < 1) { return 1; } if (days > 90) { return 90; } return days; };
    const handleCustomVariableChange = (index, field, value) => { /* ... same as v1.4.4 ... */ const updatedCustomVars = [...(currentSettings.customVariables || [])]; updatedCustomVars[index] = { ...updatedCustomVars[index], [field]: value }; setCurrentSettings(prev => ({ ...prev, customVariables: updatedCustomVars })); };
    const handleAddCustomVariable = () => { /* ... same as v1.4.4 ... */ if (!newCustomVarKey.trim()) { alert("Custom variable key cannot be empty."); return; } if (!/^[a-zA-Z0-9_]+$/.test(newCustomVarKey.trim())) { alert("Custom variable key can only contain letters, numbers, and underscores."); return; } const existingKeys = (currentSettings.customVariables || []).map(cv => cv.key); if (existingKeys.includes(newCustomVarKey.trim())) { alert("Custom variable key must be unique."); return; } const newVar = { key: newCustomVarKey.trim(), label: newCustomVarLabel.trim() || newCustomVarKey.trim() }; setCurrentSettings(prev => ({ ...prev, customVariables: [...(prev.customVariables || []), newVar] })); setNewCustomVarKey(''); setNewCustomVarLabel(''); };
    const handleRemoveCustomVariable = (index) => { /* ... same as v1.4.4 ... */ const updatedCustomVars = [...(currentSettings.customVariables || [])]; updatedCustomVars.splice(index, 1); setCurrentSettings(prev => ({ ...prev, customVariables: updatedCustomVars })); };
    const handleSave = () => { /* ... same as v1.4.4, includes surveyStatus in finalPayload ... */ const validatedSettings = { ...currentSettings }; if (validatedSettings.behaviorNavigation) { validatedSettings.behaviorNavigation = { ...validatedSettings.behaviorNavigation, saveAndContinueEmailLinkExpiryDays: getValidatedExpiryDays( validatedSettings.behaviorNavigation.saveAndContinueEmailLinkExpiryDays ), }; } else { validatedSettings.behaviorNavigation = { ...mergeWithDefaults(null).behaviorNavigation, saveAndContinueEmailLinkExpiryDays: getValidatedExpiryDays( mergeWithDefaults(null).behaviorNavigation.saveAndContinueEmailLinkExpiryDays ) }; } const settingsObjectForPayload = { completion: validatedSettings.completion || mergeWithDefaults(null).completion, accessSecurity: validatedSettings.accessSecurity || mergeWithDefaults(null).accessSecurity, behaviorNavigation: validatedSettings.behaviorNavigation, customVariables: validatedSettings.customVariables || [], appearance: validatedSettings.appearance || mergeWithDefaults(null).appearance, }; const finalPayload = { status: surveyStatus, settings: settingsObjectForPayload, }; console.log("SurveySettingsPanel: Saving data:", JSON.stringify(finalPayload, null, 2)); onSave(finalPayload); };
    
    const renderGeneralSettings = () => {
        // Add a log here too to see what surveyStatus is when this renders
        // console.log('[SurveySettingsPanel renderGeneralSettings] Dropdown value will be:', surveyStatus);
        return (
            <>
                ...
                <div style={inputGroupStyle}>
                    ...
                    <select 
                        id="surveyStatus" 
                        name="surveyStatus"
                        value={surveyStatus} // This value comes from the surveyStatus state
                        onChange={(e) => {
                            console.log('[SurveySettingsPanel Dropdown onChange] New selected value:', e.target.value);
        ...
        <div style={panelStyle}>
            ...
            <nav style={navStyle}>
                {Object.entries(SETTING_CATEGORIES).map(([key, title]) => (
                    <button key={key} onClick={() => setActiveCategory(title)} style={navButtonStyle(activeCategory === title)} > {title} </button>
                ))}
            </nav>
            <div style={contentStyle}>
                {activeCategory === SETTING_CATEGORIES.GENERAL && renderGeneralSettings()}
...
// ----- END OF MODIFIED FILE (v1.4.5 - Debugging Status Init) -----