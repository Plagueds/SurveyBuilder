...
// ----- START OF COMPLETE UPDATED FILE (v1.6 - Auto-save UI) -----
...
// Styles (assuming these are extensive, keeping them as comments for brevity, use your existing styles)
...
const customVarInputStyle = {...inputStyle, width:'38%', marginRight:'10px'}; // Adjusted width
const customVarDefaultInputStyle = {...inputStyle, width:'calc(24% - 20px)', marginRight:'10px'}; // For default value
const customVarAddRowStyle = { display: 'flex', alignItems: 'center', marginTop: '20px', paddingTop: '15px', borderTop: '1px solid #ddd' };
const customVarAddInputStyle = {...inputStyle, width:'calc(38% - 5px)', marginRight:'10px'}; // Adjusted width
const customVarAddDefaultInputStyle = {...inputStyle, width:'calc(24% - 15px)', marginRight:'10px'}; // For default value
...
const SurveySettingsPanel = ({ isOpen, onClose, initialSurveyData, onSave, surveyId }) => {
    ...
    const mergeWithDefaults = useCallback((settingsFromProps) => {
        const defaults = {
            ...
            behaviorNavigation: { 
                ...
                autoSaveEnabled: false, autoSaveIntervalSeconds: 60, // Added auto-save defaults
            },
            customVariables: [], 
            appearance: {},
        };
        ...
        for (const categoryKey in defaults) {
            if (defaults.hasOwnProperty(categoryKey)) {
                ...
                if (categoryKey === 'behaviorNavigation') {
                    ...
                    // Ensure auto-save defaults are applied if missing
    ...
    const getValidatedAutoSaveInterval = (currentSecondsValue) => {
        ...
        if (isNaN(seconds) || seconds < 15) return 15; // Min 15 seconds
        if (seconds > 300) return 300; // Max 300 seconds (5 minutes)
        return seconds;
    };
    ...
    const handleSave = () => {
        ...
        // +++ NEW: Validate autoSaveIntervalSeconds +++
        ...
        const settingsObjectForPayload = {
            ...
            behaviorNavigation: validatedSettings.behaviorNavigation, // This now includes validated auto-save settings
    ...
    const renderBehaviorNavigationSettings = () => {
        ...
        // +++ NEW: For autoSaveIntervalSeconds input +++
        ...
        return (
            <>
                ...
                {settings.questionNumberingEnabled && (
                    <>
                        <div style={inputGroupStyle}>
                         ...
                         <div style={inputGroupStyle}>
                            <label htmlFor="questionNumberingCustomPrefix" style={labelStyle}>Custom Prefix (Optional):</label>
                            <input type="text" id="questionNumberingCustomPrefix" name="questionNumberingCustomPrefix" value={settings.questionNumberingCustomPrefix || ''} onChange={(e) => handleInputChange('behaviorNavigation', e)} style={inputStyle} placeholder="e.g., Q, Section A -" />
                            <p style={subDescriptionStyle}>Prefix will appear before the number/letter (e.g., Q1, Q A).</p>
                        </div>
                    </>
                )}
                ...
                <div style={inputGroupStyle}>
                    ...
                    <label htmlFor="saveAndContinueEnabled" style={checkboxLabelStyle}>Enable "Save and Continue Later" (Manual Save)</label>
                </div>
                {settings.saveAndContinueEnabled && (
                    <>
                        <div style={inputGroupStyle}>
                            <label htmlFor="saveAndContinueMethod" style={labelStyle}>Resume Method:</label>
                            <select id="saveAndContinueMethod" name="saveAndContinueMethod" value={settings.saveAndContinueMethod || 'email'} onChange={(e) => handleInputChange('behaviorNavigation', e)} style={selectStyle} >
                        ...
                        <div style={inputGroupStyle}>
                            ...
                            <input type="number" id="saveAndContinueEmailLinkExpiryDays" name="saveAndContinueEmailLinkExpiryDays" value={displayExpiryDays} onChange={(e) => handleInputChange('behaviorNavigation', e)} min="1" max="90" style={inputStyle} placeholder="1-90" />
                            <p style={subDescriptionStyle}>Must be between 1 and 90 days.</p>
                        </div>
                    </>
                )}
                {/* +++ NEW: Auto-save Settings UI +++ */}
                {settings.saveAndContinueEnabled && ( // Auto-save is dependent on manual save being enabled
                    <>
                        <h3 style={subSectionTitleStyle}>Auto-Save on Inactivity</h3>
                        <div style={inputGroupStyle}>
                            <input type="checkbox" id="autoSaveEnabled" name="autoSaveEnabled" checked={settings.autoSaveEnabled || false} onChange={(e) => handleInputChange('behaviorNavigation', e)} style={checkboxInputStyle} />
                            <label htmlFor="autoSaveEnabled" style={checkboxLabelStyle}>Enable Auto-Save</label>
                            <p style={subDescriptionStyle}>Automatically save respondent's progress after a period of inactivity. Uses the same "Save and Continue" mechanism.</p>
                        </div>
                        {settings.autoSaveEnabled && (
                            <div style={inputGroupStyle}>
                                <label htmlFor="autoSaveIntervalSeconds" style={labelStyle}>Auto-Save After Inactivity (seconds):</label>
                                <input type="number" id="autoSaveIntervalSeconds" name="autoSaveIntervalSeconds" value={displayAutoSaveInterval} onChange={(e) => handleInputChange('behaviorNavigation', e)} min="15" max="300" style={inputStyle} placeholder="15-300" />
                                <p style={subDescriptionStyle}>Time in seconds (e.g., 60 for 1 minute). Min: 15s, Max: 300s (5 mins).</p>
                            </div>
                        )}
                    </>
                )}
    ...
    const renderCustomVariablesSettings = () => {
        ...
        return (
            <>
                ...
                <p style={subDescriptionStyle}>
                    Define keys for custom data you want to pass into the survey URL (e.g., `?source=email&id=123`).
                    This data will be stored with each response. Keys must be unique and contain only letters, numbers, and underscores.
                </p>
                <div style={{ border: '1px solid #eee', padding: '15px', borderRadius: '4px', background:'#f9f9f9' }}>
                    {customVars.length > 0 && customVars.map((cv, index) => (
                        <div key={index} style={customVarRowStyle}>
                            <input type="text" placeholder="Key (e.g., campaign_id)" value={cv.key || ''} onChange={(e) => handleCustomVariableChange(index, 'key', e.target.value)} style={customVarInputStyle} />
                            <input type="text" placeholder="Label (Optional)" value={cv.label || ''} onChange={(e) => handleCustomVariableChange(index, 'label', e.target.value)} style={customVarInputStyle} />
                            <input type="text" placeholder="Default Value (Optional)" value={cv.defaultValue || ''} onChange={(e) => handleCustomVariableChange(index, 'defaultValue', e.target.value)} style={customVarDefaultInputStyle} />
                            <button onClick={() => handleRemoveCustomVariable(index)} style={customVarRemoveButton}>Remove</button>
                        </div>
                    ))}
                    <div style={customVars.length > 0 ? customVarAddRowStyle : {display: 'flex', alignItems: 'center', marginTop: '0px'}}>
                        <input type="text" placeholder="New Key" value={newCustomVarKey} onChange={(e) => setNewCustomVarKey(e.target.value)} style={customVarAddInputStyle}/>
                        <input type="text" placeholder="New Label" value={newCustomVarLabel} onChange={(e) => setNewCustomVarLabel(e.target.value)} style={customVarAddInputStyle}/>
                        <input type="text" placeholder="New Default Value" value={newCustomVarDefaultValue} onChange={(e) => setNewCustomVarDefaultValue(e.target.value)} style={customVarAddDefaultInputStyle}/>
                        <button onClick={handleAddCustomVariable} style={customVarButton}>Add</button>
                    </div>
                </div>
            </>
        );
