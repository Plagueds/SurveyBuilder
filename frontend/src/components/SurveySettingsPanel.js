...
// ----- START OF COMPLETE UPDATED FILE (v1.5 - Custom Variables UI) -----
...
const SETTING_CATEGORIES = {
    ...
    CUSTOM_VARIABLES: 'Custom Variables', // This category will now be fully utilized
...
const customVarInputStyle = {...inputStyle, width:'40%', marginRight:'10px'};
const customVarAddRowStyle = { display: 'flex', alignItems: 'center', marginTop: '20px', paddingTop: '15px', borderTop: '1px solid #ddd' };
const customVarAddInputStyle = {...inputStyle, width:'calc(40% - 5px)', marginRight:'10px'};
...
const SurveySettingsPanel = ({ isOpen, onClose, initialSurveyData, onSave, surveyId }) => {
    ...
    const mergeWithDefaults = useCallback((settingsFromProps) => {
        const defaults = {
            ...
            behaviorNavigation: { autoAdvance: false, questionNumberingEnabled: true, questionNumberingFormat: '123', questionNumberingCustomPrefix: '', saveAndContinueEnabled: false, saveAndContinueEmailLinkExpiryDays: 7, saveAndContinueMethod: 'email', },
            customVariables: [], // Default for custom variables
            appearance: {},
        };
        const merged = {};
        for (const categoryKey in defaults) {
            if (defaults.hasOwnProperty(categoryKey)) {
                ...
                if (categoryKey === 'customVariables' && !Array.isArray(merged[categoryKey])) {
                    // Ensure customVariables is always an array, even if props are malformed
        ...
        // Ensure any top-level settings not in defaults are also carried over (if any)
    ...
    const handleInputChange = (category, e) => {
        ...
        if (type === 'checkbox') {
            ...
            if (name === 'saveAndContinueEmailLinkExpiryDays') {
                valToSet = value === '' ? '' : parseInt(value, 10); // Allow empty for temp input
            } else {
    ...
    const getValidatedExpiryDays = (currentDaysValue) => {
        ...
        if (isNaN(days) || days < 1) { return 1; } // Default to 1 if invalid or less than 1
        if (days > 90) { return 90; } // Cap at 90
        return days;
    };
    ...
    const handleAddCustomVariable = () => {
        ...
        if (!/^[a-zA-Z0-9_]+$/.test(keyTrimmed)) {
            alert("Custom variable key can only contain letters, numbers, and underscores (no spaces or special characters).");
            return;
        }
        const existingKeys = (currentSettings.customVariables || []).map(cv => cv.key);
        if (existingKeys.includes(keyTrimmed)) {
            alert("Custom variable key must be unique.");
            return;
        }
        const newVar = { key: keyTrimmed, label: newCustomVarLabel.trim() || keyTrimmed };
        setCurrentSettings(prev => ({
            ...prev,
    ...
    const handleSave = () => {
        ...
        // Ensure behaviorNavigation exists and validate expiry days
        validatedSettings.behaviorNavigation = {
            ...mergeWithDefaults(null).behaviorNavigation, // Start with defaults
            ...(validatedSettings.behaviorNavigation || {}), // Overlay current values
        };
        ...
        const finalPayload = {
            status: surveyStatus, // surveyStatus state variable
            settings: settingsObjectForPayload,
        };
        console.log("SurveySettingsPanel: Saving data:", JSON.stringify(finalPayload, null, 2));
        onSave(finalPayload); // onSave expects the whole survey data structure or parts of it
    };
        ...
        return (
            <>
                ...
                <div style={inputGroupStyle}>
                    ...
                    <input type="datetime-local" id="linkExpirationDate" name="linkExpirationDate" 
                           value={settings.linkExpirationDate ? settings.linkExpirationDate.substring(0, 16) : ''} // Format for datetime-local
                           onChange={(e) => handleInputChange('accessSecurity', e)} style={inputStyle} />
                </div>
                <div style={inputGroupStyle}>
                    ...
                    <>
                        <div style={inputGroupStyle}>
                            ...
                            <select id="questionNumberingFormat" name="questionNumberingFormat" value={settings.questionNumberingFormat || '123'} onChange={(e) => handleInputChange('behaviorNavigation', e)} style={selectStyle} >
                                ...
                                {/* <option value="custom">Custom Prefix</option> */} {/* Custom prefix might be a separate field */}
                            </select>
                        </div>
                         <div style={inputGroupStyle}>
                            ...
                            <input
                                type="text"
                                id="questionNumberingCustomPrefix"
                                name="questionNumberingCustomPrefix"
                                value={settings.questionNumberingCustomPrefix || ''}
                                onChange={(e) => handleInputChange('behaviorNavigation', e)}
                                style={inputStyle}
                                placeholder="e.g., Q, Section A -"
                            />
            ...
            <>
                ...
                <div style={{ border: '1px solid #eee', padding: '15px', borderRadius: '4px', background:'#f9f9f9' }}>
                    {customVars.length > 0 && customVars.map((cv, index) => (
                        <div key={index} style={customVarRowStyle}>
                            <input
                                ...
                                style={customVarInputStyle}
                                // readOnly // Keys should ideally be immutable once responses exist, but for now allow editing
                            />
                            <input
                                type="text"
                                placeholder="Label (Optional Description)"
                                value={cv.label || ''}
                                onChange={(e) => handleCustomVariableChange(index, 'label', e.target.value)}
                                style={customVarInputStyle}
                            />
                            <button onClick={() => handleRemoveCustomVariable(index)} style={customVarRemoveButton}>
                                Remove
                            </button>
                        </div>
                    ))}
                    <div style={customVars.length > 0 ? customVarAddRowStyle : {display: 'flex', alignItems: 'center', marginTop: '0px'}}>
                        <input
                            type="text"
                            placeholder="New Key (no spaces)"
                            value={newCustomVarKey}
                            onChange={(e) => setNewCustomVarKey(e.target.value)}
                            style={customVarAddInputStyle}
                        />
                        <input
                            type="text"
                            placeholder="New Label (Optional)"
                            value={newCustomVarLabel}
                            onChange={(e) => setNewCustomVarLabel(e.target.value)}
                            style={customVarAddInputStyle}
                        />
                        <button onClick={handleAddCustomVariable} style={customVarButton}>
                            Add Variable
                        </button>
                    </div>
                </div>
...
// ----- END OF COMPLETE UPDATED FILE (v1.5 - Custom Variables UI) -----