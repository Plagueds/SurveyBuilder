...
// ----- START OF COMPLETE MODIFIED FILE (v1.13.1 - Fix Rules of Hooks error) -----
...
// (evaluateCondition, evaluateLogicGroup, evaluateRuleConditions, getDynamicallyVisibleQuestionIds - same as v1.13)
function evaluateCondition(condition, currentAnswers, questionsById, otherInputValues = {}) {
    ...
    const isAnswerEmpty = (ans) => {
        if (Array.isArray(ans)) return ans.length === 0;
        if (typeof ans === 'object' && ans !== null) {
            if (sourceQuestion.type === 'heatmap') return !ans.clicks || ans.clicks.length === 0;
            if (sourceQuestion.type === 'cardsort') return !ans.assignments || Object.keys(ans.assignments).length === 0;
        }
    ...
    if (isAnswerEmpty(answerValue) && 
        !['ne', 'notContains'].includes(condition.conditionOperator) 
    ) {
        return false;
    }
    ...
    switch (sourceQuestion.type) {
        ...
        case 'matrix':
            ...
            const matrixRowAns = answerValue[condition.conditionValue?.split(';')[0]]; 
            ...
            switch (condition.conditionOperator) {
                ...
                case 'rowIsAnswered': 
                    ...
                    return matrixRowAns !== undefined && matrixRowAns !== null && String(matrixRowAns).trim() !== '';
            ...
            switch (condition.conditionOperator) {
                ...
                case 'categoryHasCards': 
                    if (conditionValStr === UNASSIGNED_CARD_SORT_CATEGORY_ID) {
                        const allCardsInQuestion = sourceQuestion.options?.map(opt => opt.id || opt.value || opt) || []; 
                        return allCardsInQuestion.some(cardId => !assignments[cardId] || assignments[cardId] === UNASSIGNED_CARD_SORT_CATEGORY_ID);
                    }
                    for (const cardId in assignments) {
                        if (assignments.hasOwnProperty(cardId) && String(assignments[cardId]) === conditionValStr) {
                            return true; 
                        }
                    }
                    return false;
...
function SurveyTakingPage() {
    ...
    // +++ MOVED useMemo for orderedVisibleForOnePerPage to top level +++
    ...
    const showNextButton = questionDisplayMode === 'onePerPage' && !isSubmitState && orderedVisibleForOnePerPage.length > 0;
    const showSubmitButtonLogic = isSubmitState && ( (questionDisplayMode === 'onePerPage' && currentQuestionToRenderOnePerPage) || (questionDisplayMode === 'allOnOnePage' && allVisibleQuestionsToRender.length > 0) || (Object.keys(currentAnswers).length > 0 && dynamicallyVisibleQuestionIds.size === 0 && originalQuestions.length > 0) );
            ...
            <main className={styles.surveyQuestionArea}>
                ...
                { !isLoadingSurvey && !isSubmitting &&
                    ( (questionDisplayMode === 'onePerPage' && !currentQuestionToRenderOnePerPage && orderedVisibleForOnePerPage.length > 0 && currentVisibleIndex >= orderedVisibleForOnePerPage.length) || 
                      ...
                      (dynamicallyVisibleQuestionIds.size === 0 && originalQuestions.length > 0) 
                    ) &&
...
// ----- END OF COMPLETE MODIFIED FILE (v1.13.1 - Fix Rules of Hooks error) -----