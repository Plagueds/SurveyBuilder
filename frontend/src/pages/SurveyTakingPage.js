...
// ----- START OF COMPLETE MODIFIED FILE (v1.9 - Handle Custom Variables) -----
...
// Question component imports (assuming they are correct)
...
// Helper functions (assuming they are correct)
...
function SurveyTakingPage() {
    ...
    // +++ NEW: State for captured custom variables +++
    const [capturedCustomVariables, setCapturedCustomVariables] = useState({});

    useEffect(() => { let c = sessionStorage.getItem(`surveyClientSessionId_${surveyId}_${collectorId}`); if(!c){c=uuidv4(); sessionStorage.setItem(`surveyClientSessionId_${surveyId}_${collectorId}`,c); console.log('[SurveyTakingPage] New clientSessionId generated:', c);} else {console.log('[SurveyTakingPage] Using existing clientSessionId:', c);} setClientSessionId(c);},[surveyId, collectorId]);
    useEffect(()=>{const s=location.state; if(s){if(s.surveyTitle && s.surveyTitle!==initialSurveyTitle)setInitialSurveyTitle(s.surveyTitle); if(s.collectorSettings){const n=JSON.stringify(s.collectorSettings),c=JSON.stringify(collectorSettings); if(n!==c){setCollectorSettings(s.collectorSettings);if(recaptchaRef.current)recaptchaRef.current.reset();setRecaptchaToken(null);}}}}, [location.state, initialSurveyTitle, collectorSettings]); // Simplified dependencies
    ...
    useEffect(() => { return () => { if (autoAdvanceTimeoutRef.current) { clearTimeout(autoAdvanceTimeoutRef.current); } }; }, []);
    ...
    useEffect(() => { 
        ...
        // Pass all query params to backend for custom variable parsing
        Object.keys(queryParamsObject).forEach(key => {
            fetchOptions[key] = queryParamsObject[key];
        });
        ...
        console.log(`[SurveyTakingPage Fetch] Options for getSurveyById (includes query params):`, JSON.stringify(fetchOptions));
        ...
        surveyApi.getSurveyById(surveyId, { ...fetchOptions, signal: abortController.signal })
            .then(response => {
                if (response.success && response.data) {
                    ...
                    const mergedSettings = { /* ... same as v1.8 ... */ ...surveyBehaviorNav, ...apiCollectorSettings, progressBarEnabled: apiCollectorSettings.progressBarEnabled ?? surveyBehaviorNav.progressBarEnabled ?? false, progressBarStyle: apiCollectorSettings.progressBarStyle || surveyBehaviorNav.progressBarStyle || 'percentage', progressBarPosition: apiCollectorSettings.progressBarPosition || surveyBehaviorNav.progressBarPosition || 'top', allowResume: apiCollectorSettings.saveAndContinueEnabled ?? surveyBehaviorNav.saveAndContinueEnabled ?? false, saveAndContinueMethod: apiCollectorSettings.saveAndContinueMethod || surveyBehaviorNav.saveAndContinueMethod || 'both', questionNumberingFormat: apiCollectorSettings.questionNumberingFormat || surveyBehaviorNav.questionNumberingFormat || '123', questionNumberingCustomPrefix: apiCollectorSettings.questionNumberingCustomPrefix || surveyBehaviorNav.questionNumberingCustomPrefix || '', };
                    setCollectorSettings(mergedSettings);

                    // +++ MODIFIED: Handle custom variables +++
                    ...
                    if (effectivePartialResponse && typeof effectivePartialResponse === 'object' && Object.keys(effectivePartialResponse).length > 0) {
                        ...
                        // Custom variables from partial response (already merged by backend if resuming with code)
                        setCapturedCustomVariables(effectivePartialResponse.customVariables || {});
                        console.log("[SurveyTakingPage] Set capturedCustomVariables from partial:", JSON.stringify(effectivePartialResponse.customVariables));
                    ...
                    } else if (response.data.initialCustomVariables) { // New session, use initial custom vars from backend
                        setCapturedCustomVariables(response.data.initialCustomVariables || {});
                        console.log("[SurveyTakingPage] Set capturedCustomVariables from initial:", JSON.stringify(response.data.initialCustomVariables));
                        setCurrentAnswers({}); // Ensure answers are fresh for new session with custom vars
                        setOtherInputValues({});
                        setCurrentVisibleIndex(0);
                    } else if (fetchOptions.resumeToken && !isResumingWithCodeFromState) { 
                        console.log(`[SurveyTakingPage] Resume token ${fetchOptions.resumeToken} provided in URL, but no valid partial response data found. Starting fresh.`);
                        setCurrentResumeToken(null); setCurrentAnswers({}); setOtherInputValues({}); setCurrentVisibleIndex(0); 
                        setCapturedCustomVariables({}); // Clear custom vars too
                        toast.info("Could not resume previous session. Starting a new session.", { autoClose: 7000 }); 
                    } else {
                        console.log("[SurveyTakingPage] No effective partial response or initial custom vars. Starting fresh.");
    ...
    const handleSubmit = useCallback(async () => {
        ...
        const payloadToSubmit = { 
            ...
            customVariables: capturedCustomVariables // Add captured custom variables
        };
        console.log("[SurveyTakingPage handleSubmit] Payload:", JSON.stringify(payloadToSubmit));
        try {
            const result = await surveyApi.submitSurveyAnswers(surveyId, payloadToSubmit);
            if (result && result.success) { /* ... same as v1.8 ... */ toast.success("Survey submitted successfully!"); sessionStorage.removeItem(`surveyClientSessionId_${surveyId}_${collectorId}`); if (recaptchaRef.current) recaptchaRef.current.reset(); setRecaptchaToken(null); navigate(`/thank-you`, {state: {surveyTitle: survey?.title || initialSurveyTitle, thankYouMessage: result.thankYouMessage}}); } 
            else { /* ... same as v1.8 ... */ setSubmissionError(result.message || "An unknown error occurred during submission."); if (recaptchaRef.current) recaptchaRef.current.reset(); setRecaptchaToken(null); }
        } catch (err) { /* ... same as v1.8 ... */ setSubmissionError(err.message || "An unexpected error occurred."); if (recaptchaRef.current) recaptchaRef.current.reset(); setRecaptchaToken(null); } 
        finally { setIsSubmitting(false); }
    }, [surveyId, collectorId, currentAnswers, otherInputValues, currentResumeToken, navigate, survey?.title, initialSurveyTitle, isSubmitting, clientSessionId, collectorSettings, recaptchaToken, currentQuestionToRender, validateQuestion, capturedCustomVariables]); // Added capturedCustomVariables

    const handleNext = useCallback(() => { /* ... same as v1.8 ... */ if (autoAdvanceTimeoutRef.current) clearTimeout(autoAdvanceTimeoutRef.current); if (currentQuestionToRender && !validateQuestion(currentQuestionToRender)) { return; } setShowValidationModal(false); setValidationModalMessage(''); if (!isSubmitState) { setCurrentVisibleIndex(prev => prev + 1); } else { handleSubmit(); } }, [isSubmitState, validateQuestion, currentQuestionToRender, handleSubmit]);
    const handleInputChange = useCallback((questionId, value) => { /* ... same as v1.8 ... */ setCurrentAnswers(prev => ({ ...prev, [questionId]: value })); setShowValidationModal(false); setValidationModalMessage(''); const autoAdvanceEnabled = collectorSettings?.autoAdvance ?? survey?.settings?.behaviorNavigation?.autoAdvance ?? false; const question = questionsById[questionId]; const autoAdvanceTypes = ['multiple-choice', 'nps', 'rating']; const isOtherSelectedForOtherQuestion = question && question.addOtherOption && value === OTHER_VALUE_INTERNAL; if (autoAdvanceEnabled && question && autoAdvanceTypes.includes(question.type) && !isSubmitState && !isOtherSelectedForOtherQuestion) { if (autoAdvanceTimeoutRef.current) clearTimeout(autoAdvanceTimeoutRef.current); autoAdvanceTimeoutRef.current = setTimeout(() => { handleNext(); autoAdvanceTimeoutRef.current = null; }, 500); } else if (autoAdvanceTimeoutRef.current && isOtherSelectedForOtherQuestion) { clearTimeout(autoAdvanceTimeoutRef.current); autoAdvanceTimeoutRef.current = null; } }, [collectorSettings, survey, questionsById, isSubmitState, handleNext, OTHER_VALUE_INTERNAL]);
    const handleCheckboxChange = useCallback((questionId, optionValue, isChecked) => { /* ... same as v1.8 ... */ setCurrentAnswers(prevAnswers => { const currentSelection = prevAnswers[questionId] ? [...ensureArray(prevAnswers[questionId])] : []; let newSelection = isChecked ? [...currentSelection, optionValue] : currentSelection.filter(val => val !== optionValue); if (optionValue === OTHER_VALUE_INTERNAL && !isChecked) { setOtherInputValues(prevOther => ({ ...prevOther, [`${questionId}_other`]: '' })); } return { ...prevAnswers, [questionId]: newSelection }; }); setShowValidationModal(false); setValidationModalMessage(''); }, [OTHER_VALUE_INTERNAL]);
    const handleOtherInputChange = useCallback((questionId, value) => { /* ... same as v1.8 ... */ setOtherInputValues(prev => ({ ...prev, [`${questionId}_other`]: value })); setCurrentAnswers(prevAnswers => { const currentSelection = prevAnswers[questionId] ? [...ensureArray(prevAnswers[questionId])] : []; if (value && value.trim() !== '' && !currentSelection.includes(OTHER_VALUE_INTERNAL)) { return { ...prevAnswers, [questionId]: [...currentSelection, OTHER_VALUE_INTERNAL] }; } return prevAnswers; }); setShowValidationModal(false); setValidationModalMessage(''); }, [OTHER_VALUE_INTERNAL]);
    const handleComplexAnswerChange = useCallback((questionId, structuredAnswer) => { /* ... same as v1.8 ... */ setCurrentAnswers(prev => ({ ...prev, [questionId]: structuredAnswer })); setShowValidationModal(false); setValidationModalMessage(''); }, []);
    const handlePrevious = useCallback(() => { /* ... same as v1.8 ... */ if (autoAdvanceTimeoutRef.current) clearTimeout(autoAdvanceTimeoutRef.current); setShowValidationModal(false); setValidationModalMessage(''); setCurrentVisibleIndex(prevIndex => (prevIndex > 0 ? prevIndex - 1 : prevIndex)); }, []);
    
    const performSaveAndContinue = useCallback(async (emailForSave = null) => {
        if (!surveyId || !collectorId) { toast.error("Cannot save: IDs missing."); return; }
        let currentSaveClientSessionId = clientSessionId;
        if (!currentSaveClientSessionId) { /* ... same fallback as v1.8 ... */ currentSaveClientSessionId = uuidv4(); setClientSessionId(currentSaveClientSessionId); sessionStorage.setItem(`surveyClientSessionId_${surveyId}_${collectorId}`, currentSaveClientSessionId); }
