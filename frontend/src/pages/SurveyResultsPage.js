...
const CARD_SORT_UNASSIGNED_ID = '__UNASSIGNED_CARDS__';
const NA_KEY = '__NA__';
                    ...
                    <div>
                        ...
                        {Object.entries(levelCounts).map(([attrName, levels]) => (
                            <div key={attrName} style={{ marginBottom: '15px' }}>
                                ...
                                <table style={{...styles.resultsTable, width: 'auto', minWidth: '300px'}}>
                                    <thead>
                                        <tr>
                                            <th style={styles.resultsTableTh}>Level</th>
                                            <th style={styles.resultsTableTh}>Count</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {Object.entries(levels).sort((a,b) => b[1] - a[1]).map(([levelName, count]) => (
                                            <tr key={levelName}>
                                                <td style={styles.resultsTableCellValue}>{levelName}</td>
                                                <td style={styles.resultsTableCellCount}>{count}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
            ...
            case 'cardsort': {
                ...
                return ( <div style={styles.cardSortResultContainer}><p style={styles.infoText}>Card placements across {qRespondents} respondents.</p>{uniqueDisplayCategories.map(({ id: categoryId, name: categoryName }) => {
                    ...
                    const sortedCards = Object.entries(cardsInCategory).map(([cardId, count]) => ({
                        cardText: cardIdToTextMap[cardId] || cardId,
                        count
