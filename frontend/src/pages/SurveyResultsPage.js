...
const getChartColor = (index, alpha = 0.8) => {
    ...
    if (alpha < 1 && color.startsWith('#') && color.length === 7) {
        try {
            ...
            console.warn("Color parsing error in getChartColor for:", color, e);
    ...
    console.warn("Unhandled color format in getChartColor:", color);
    return 'rgba(128,128,128,0.5)';
};
...
const createHistogramData = (values, minVal, maxVal, numBins = 10) => {
    if (!values || values.length === 0) return { labels: [], data: [] };
    const cleanValues = values.map(v => parseFloat(v)).filter(v => !isNaN(v));
    if (cleanValues.length === 0) return { labels: [], data: [] };
    const actualMin = minVal !== undefined && minVal !== null && !isNaN(parseFloat(minVal)) ? parseFloat(minVal) : Math.min(...cleanValues);
    const actualMax = maxVal !== undefined && maxVal !== null && !isNaN(parseFloat(maxVal)) ? parseFloat(maxVal) : Math.max(...cleanValues);
    if (actualMin === actualMax) { return { labels: [actualMin.toFixed(1)], data: [cleanValues.length] }; }
    const range = actualMax - actualMin;
    if (range < 0) { console.warn("Histogram range is negative", {actualMin, actualMax, range}); return { labels: [], data: [] }; }
    const binSize = range > 0 ? range / numBins : 1;
    const bins = Array(numBins).fill(0);
    const labels = [];
    for (let i = 0; i < numBins; i++) { const binMin = actualMin + i * binSize; const binMax = actualMin + (i + 1) * binSize; labels.push(`${binMin.toFixed(1)}-${binMax.toFixed(1)}`); }
    cleanValues.forEach(value => { if (value < actualMin || value > actualMax) return; let binIndex = Math.floor((value - actualMin) / binSize); if (value === actualMax) binIndex = numBins - 1; binIndex = Math.max(0, Math.min(numBins - 1, binIndex)); bins[binIndex]++; });
    return { labels, data: bins };
};

const safeJsonParse = (value, defaultValue = null) => {
    if (typeof value !== 'string' || !value.trim()) { return defaultValue; }
    try { const parsed = JSON.parse(value); return parsed; }
    catch (e) { console.warn("Failed to parse JSON answer value:", value, e); return defaultValue; }
};
...
function SurveyResultsPage() {
    ...
    const processAllAnswers = useCallback((surveyDefinition, allAnswers) => {
        ...
        const results = {};
        const respondentSessionIds = new Set(allAnswers.map(a => a.sessionId));
        ...
        surveyDefinition.questions.forEach(question => {
            ...
            const questionRespondents = new Set(questionAnswers.map(a => a.sessionId)).size;
            let stats = {
                ...
                switch (questionType) {
                    case 'multiple-choice': case 'dropdown': case 'checkbox':
                        ...
                        questionAnswers.forEach(ans => {
                            const values = questionType === 'checkbox' ? (ans.answerValue || '').split('||').filter(Boolean) : [ans.answerValue];
                            values.forEach(val => {
                                if (val === null || val === undefined || val === '') return;
                                if (val === NA_KEY) { tempCounts[NA_KEY] = (tempCounts[NA_KEY] || 0) + 1; }
                                else if (String(val).startsWith('Other: ')) { const writeInText = String(val).substring(7).trim(); tempCounts[OTHER_KEY_INTERNAL] = (tempCounts[OTHER_KEY_INTERNAL] || 0) + 1; if (writeInText) { stats.writeIns[writeInText] = (stats.writeIns[writeInText] || 0) + 1; } }
                                else if (question.options?.includes(val) || val === OTHER_KEY_INTERNAL) { tempCounts[val] = (tempCounts[val] || 0) + 1; }
                                else { console.warn(`[${question.text}] Unexpected value for ${questionType}:`, val, "Available options:", question.options); }
                            });
                        });
                        stats.counts = tempCounts;
                        break;
                    case 'rating': case 'nps': case 'slider':
                        const numericValues = questionAnswers.map(ans => parseFloat(ans.answerValue)).filter(val => !isNaN(val));
                        if (numericValues.length > 0) {
                            ...
                            if (questionType === 'nps') {
                                ...
                                const totalNPS = stats.promoters + stats.passives + stats.detractors;
                                stats.npsScore = totalNPS > 0 ? ((stats.promoters / totalNPS) * 100 - (stats.detractors / totalNPS) * 100) : 0;
                            }
                        }
                        break;
                    case 'text': case 'textarea':
                        stats.responses = questionAnswers.map(ans => ans.answerValue).filter(val => val !== null && val !== undefined && String(val).trim() !== '');
                        break;
                    case 'matrix':
                        stats.rows = {};
                    ...
                    case 'ranking':
                        ...
                        questionAnswers.forEach(ans => {
                            const rankedList = safeJsonParse(ans.answerValue, []);
                            if (!Array.isArray(rankedList)) return;
                            rankedList.forEach((option, index) => {
                                if (!rankOptions.includes(option)) return;
                                const rankPosition = index + 1;
                                if (!stats.averageRanks[option]) { stats.averageRanks[option] = { sum: 0, count: 0 }; }
                                stats.averageRanks[option].sum += rankPosition; stats.averageRanks[option].count++;
                    ...
                    case 'heatmap':
                        stats.clicks = [];
                        questionAnswers.forEach(ans => {
                            const clicksData = safeJsonParse(ans.answerValue, []);
                    ...
                    case 'maxdiff':
                        ...
                        questionAnswers.forEach(ans => {
                            const mdData = safeJsonParse(ans.answerValue, {});
                    ...
                    case 'conjoint':
                        stats.levelCounts = {};
                        questionAnswers.forEach(ans => {
                            const chosenProfile = safeJsonParse(ans.answerValue, {});
                            ...
                            Object.entries(assignments).forEach(([cardId, categoryId]) => {
                                ...
                                if (stats.cardPlacementsByCategory[categoryId]) {
                                   stats.cardPlacementsByCategory[categoryId][cardId] = (stats.cardPlacementsByCategory[categoryId][cardId] || 0) + 1;
                                    ...
                                    console.warn(`Card "${cardId}" assigned to an unknown/deleted category "${categoryId}". Placing in Unassigned.`);
                                    if (!stats.cardPlacementsByCategory[CARD_SORT_UNASSIGNED_ID]) stats.cardPlacementsByCategory[CARD_SORT_UNASSIGNED_ID] = {};
                                    stats.cardPlacementsByCategory[CARD_SORT_UNASSIGNED_ID][cardId] = (stats.cardPlacementsByCategory[CARD_SORT_UNASSIGNED_ID][cardId] || 0) + 1;
                                }
                            });
                        });
                        break;
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (!surveyResponse.ok) {
                ...
                console.error(`Survey definition fetch failed with status ${surveyResponse.status}:`, errorBody);
                let errorMessage = `Failed to fetch survey definition (Status: ${surveyResponse.status}).`;
                if (surveyResponse.status === 401) {
                    errorMessage = "Unauthorized to view survey definition. Your session might have expired.";
                } else if (surveyResponse.status === 403) {
                    errorMessage = "Forbidden: You do not have permission to access this survey's definition.";
            ...
            console.log("Survey definition JSON response:", surveyJsonResponse);
            ...
            if (!answersResponse.ok) {
                ...
                console.error(`Survey answers fetch failed with status ${answersResponse.status}:`, errorBody);
                let errorMessage = `Failed to fetch survey answers (Status: ${answersResponse.status}).`;
                 if (answersResponse.status === 401) {
                    errorMessage = "Unauthorized to view survey answers. Your session might have expired.";
                } else if (answersResponse.status === 403) {
                    errorMessage = "Forbidden: You do not have permission to access these survey answers.";
                }
                try {
                    const parsedError = JSON.parse(errorBody);
                    if (parsedError.message) errorMessage += ` Server: ${parsedError.message}`;
            ...
            const answersJsonResponse = await answersResponse.json();
    ...
    }, [survey, rawAnswers, loading, processAllAnswers]);
    ...
    const renderQuestionResults = (question) => {
        ...
        if (stats?.processingError) { return <p style={{...styles.noAnswerText, color: 'red'}}>Error processing results: {stats.processingError}</p>; }
        const qRespondents = stats?.totalResponses || 0;
        if (!stats || qRespondents === 0) { return <p style={styles.noAnswerText}>No answers submitted for this question yet.</p>; }

        const basePlugins = { legend: { display: true, position: 'bottom', labels: { boxWidth: 12, padding: 20 } }, title: { display: false }, tooltip: { callbacks: { label: function(context) { let label = context.dataset.label || context.label || ''; if (label) { label += ': '; } if (context.parsed !== null && context.parsed !== undefined) { const value = typeof context.parsed === 'object' ? context.parsed.y : context.parsed; const dataArr = context.dataset.data; const chartTotal = dataArr.reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0); label += `${value} (${calculatePercentage(value, chartTotal)})`; } return label; } } } };
        const pieChartOptions = { responsive: true, maintainAspectRatio: false, plugins: { ...basePlugins, legend: { ...basePlugins.legend, position: 'right' } } };
        const barChartOptions = { responsive: true, maintainAspectRatio: false, plugins: basePlugins, scales: { y: { beginAtZero: true, ticks: { precision: 0 } }, x: {} } };
        ...
        switch (type) {
            case 'multiple-choice': case 'dropdown': case 'checkbox': {
                ...
                if (counts[NA_KEY] > 0) { const item = { key: NA_KEY, displayLabel: 'Not Applicable', count: counts[NA_KEY], type: 'na' }; chartItems.push(item); tableItems.push(item); }
                ...
                return ( <div style={styles.resultContainer}> {chartItems.length > 0 && <div style={isPieChart ? styles.chartContainerPie : styles.chartContainerBar}>{isPieChart ? <Pie options={currentChartOptions} data={chartData} /> : <Bar options={currentChartOptions} data={chartData} />}</div>} {tableItems.length > 0 && <table style={styles.resultsTable}><thead><tr><th style={styles.resultsTableTh}>Value</th><th style={styles.resultsTableTh}>Percent</th><th style={styles.resultsTableTh}>Responses</th></tr></thead><tbody>{tableItems.map((item, i) => (<tr key={`${item.type}-${item.key}`}><td>{item.displayLabel}</td><td>{calculatePercentage(item.count, totalForTablePercent)}<div style={styles.percentBarContainer}><div style={{...styles.percentBar, width: calculatePercentage(item.count, totalForTablePercent), backgroundColor: getSolidChartColor(i)}}></div></div></td><td>{item.count}</td></tr>))}</tbody>{(type === 'multiple-choice' || type === 'dropdown') && <tfoot><tr><td colSpan="2" style={{textAlign: 'right'}}>Total:</td><td>{totalForTablePercent}</td></tr></tfoot>}{type === 'checkbox' && <tfoot><tr><td colSpan="2" style={{textAlign: 'right'}}>Total Respondents:</td><td>{qRespondents}</td></tr></tfoot>}</table>} {writeInDetails.length > 0 && <div style={styles.writeInContainer}><strong style={styles.writeInHeader}>"{OTHER_DISPLAY_LABEL}" Responses:</strong><ul style={styles.writeInList}>{writeInDetails.map((detail, index) => (<li key={index} style={styles.writeInItem}>"{detail.text}" <span style={styles.writeInCount}>({detail.count})</span></li>))}</ul></div>} </div> );
            }
            case 'rating': {
                const ratingLabels = (options.length > 0 ? options.map((_, i) => String(i + 1)) : [1, 2, 3, 4, 5].map(String));
                const ratingCounts = ratingLabels.map(score => stats.counts?.[String(score)] || 0);
                const avgRating = stats.average !== null ? Number(stats.average).toFixed(2) : 'N/A';
                const backgroundColors = ratingLabels.map((_, i) => getChartColor(i, 0.8));
                const borderColors = ratingLabels.map((_, i) => getSolidChartColor(i));
                if (ratingLabels.length === 0 || ratingCounts.some(v => typeof v !== 'number' || isNaN(v)) || backgroundColors.some(c => typeof c !== 'string') || borderColors.some(c => typeof c !== 'string')) { return <p style={styles.noAnswerText}>Chart data incomplete for rating.</p>; }
                const chartDataRating = { labels: ratingLabels, datasets: [{ label: '# Responses', data: ratingCounts, backgroundColor: backgroundColors, borderColor: borderColors, borderWidth: 1 }] };
                return ( <div style={styles.resultContainer}><p style={styles.summaryStat}>Average Rating: {avgRating}</p><div style={styles.chartContainerBar}><Bar options={barChartOptions} data={chartDataRating} /></div></div> );
            }
            case 'nps': {
                const { promoters = 0, passives = 0, detractors = 0, npsScore = 0 } = stats;
                const totalNPSResponses = qRespondents;
                const promoterPercent = calculatePercentage(promoters, totalNPSResponses);
                const passivePercent = calculatePercentage(passives, totalNPSResponses);
                const detractorPercent = calculatePercentage(detractors, totalNPSResponses);
                const npsScoreLabels = Object.keys(stats.counts || {}).map(k => parseInt(k)).sort((a, b) => a - b).map(String);
                const npsScoreCounts = npsScoreLabels.map(score => stats.counts[score] || 0);
                const npsBackgroundColors = npsScoreLabels.map(score => { const s = parseInt(score); if (s >= 9) return styles.npsPromoterColor.backgroundColor; if (s >= 7) return styles.npsPassiveColor.backgroundColor; return styles.npsDetractorColor.backgroundColor; });
                if (npsScoreLabels.length === 0 || npsScoreCounts.some(v => typeof v !== 'number' || isNaN(v)) || npsBackgroundColors.some(c => typeof c !== 'string')) { return <p style={styles.noAnswerText}>NPS score distribution data incomplete.</p>; }
                const chartDataNpsScores = { labels: npsScoreLabels, datasets: [{ label: '# Responses', data: npsScoreCounts, backgroundColor: npsBackgroundColors, borderWidth: 1 }] };
                const npsBarOptions = { ...barChartOptions, plugins: { ...barChartOptions.plugins, legend: { display: false } } };
            ...
            case 'slider': {
                const sliderValues = stats.values || [];
                const avgSlider = stats.average !== null ? Number(stats.average).toFixed(2) : 'N/A';
                const minQVal = sliderMin !== undefined && sliderMin !== null ? parseFloat(sliderMin) : null;
                const maxQVal = sliderMax !== undefined && sliderMax !== null ? parseFloat(sliderMax) : null;
                const histData = createHistogramData(sliderValues, minQVal, maxQVal, 10);
                const backgroundColors = histData.labels.map((_, i) => getChartColor(i, 0.8));
                const borderColors = histData.labels.map((_, i) => getSolidChartColor(i));
                if (histData.labels.length === 0 || histData.data.some(v => typeof v !== 'number' || isNaN(v)) || backgroundColors.some(c => typeof c !== 'string') || borderColors.some(c => typeof c !== 'string')) { return <p style={styles.noAnswerText}>Slider histogram data incomplete.</p>; }
                const chartDataSlider = { labels: histData.labels, datasets: [{ label: '# Responses', data: histData.data, backgroundColor: backgroundColors, borderColor: borderColors, borderWidth: 1, barPercentage: 1.0, categoryPercentage: 1.0, }] };
                const sliderBarOptions = { ...barChartOptions, plugins: { ...barChartOptions.plugins, legend: { display: false } }, scales: { x: { grid: { display: false }, title: { display: true, text: `Range (${minQVal ?? 'Auto'}-${maxQVal ?? 'Auto'})`} }, y: { ticks: { precision: 0 }, title: { display: true, text: 'Frequency' } } } };
                return ( <div style={styles.resultContainer}><p style={styles.summaryStat}>Average: {avgSlider} (Min: {stats.min ?? 'N/A'}, Max: {stats.max ?? 'N/A'})</p><div style={styles.chartContainerBar}><Bar options={sliderBarOptions} data={chartDataSlider} /></div></div> );
            }
            ...
            case 'cardsort': {
                ...
                displayCategories.forEach(cat => { if (!uniqueDisplayCategoriesMap.has(cat.id)) { uniqueDisplayCategoriesMap.set(cat.id, cat); } });
    ...
    // --- Styles ---
    const styles = {
        pageContainer: { padding: '20px', maxWidth: '950px', margin: 'auto', fontFamily: 'Arial, sans-serif', color: '#333' },
        header: { borderBottom: '1px solid #eee', paddingBottom: '10px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' },
        surveyTitle: { margin: 0, fontSize: '1.8em', flexGrow: 1 },
        respondentCount: { fontSize: '1.1em', fontWeight: 'bold', color: '#6c757d', whiteSpace: 'nowrap' },
        questionResultBox: { marginBottom: '40px', padding: '20px', border: '1px solid #dee2e6', borderRadius: '8px', backgroundColor: '#fff' },
        questionText: { fontWeight: 'normal', fontSize: '1.3em', marginBottom: '20px', color: '#212529' },
        resultContainer: { display: 'flex', flexDirection: 'column', gap: '20px' },
        chartContainerPie: { height: '280px', width: '100%', maxWidth: '450px', margin: '0 auto 15px auto', position: 'relative' },
        chartContainerBar: { height: '300px', width: '100%', marginBottom: '15px', position: 'relative' },
        resultsTable: { width: '100%', borderCollapse: 'collapse', marginTop: '10px', fontSize: '0.9em', border: '1px solid #dee2e6' },
        resultsTableTh: { backgroundColor: '#f8f9fa', fontWeight: '600', padding: '10px 12px', border: '1px solid #dee2e6', textAlign: 'left' },
        resultsTableCell: { border: '1px solid #dee2e6', padding: '8px 12px', textAlign: 'center', verticalAlign: 'middle' },
        resultsTableCellValue: { textAlign: 'left', padding: '8px 12px', border: '1px solid #dee2e6', verticalAlign: 'middle' },
        resultsTableCellPercent: { textAlign: 'left', padding: '8px 12px', border: '1px solid #dee2e6', verticalAlign: 'middle', width: '150px' },
        resultsTableCellCount: { textAlign: 'right', padding: '8px 12px', border: '1px solid #dee2e6', verticalAlign: 'middle', fontWeight: '500' },
        percentBarContainer: { width: '100%', backgroundColor: '#e9ecef', height: '10px', borderRadius: '3px', marginTop: '4px', overflow: 'hidden' },
        percentBar: { height: '100%', transition: 'width 0.3s ease-in-out' },
        summaryStat: { fontWeight: 'bold', fontSize: '1.1em', margin: '10px 0', color: '#0d6efd' },
        infoText: { fontSize: '0.9em', color: '#6c757d', marginBottom: '10px' },
        noAnswerText: { fontStyle: 'italic', color: '#6c757d' },
        textResponseList: { listStyle: 'none', padding: 0, maxHeight: '300px', overflowY: 'auto', border: '1px solid #dee2e6', borderRadius: '4px' },
        textResponseItem: { padding: '8px 12px', borderBottom: '1px dotted #dee2e6' },
        rowHeader: { fontWeight: 'bold', textAlign: 'left', padding: '8px 12px', border: '1px solid #dee2e6', verticalAlign: 'middle', backgroundColor: '#f8f9fa' },
        heatmapContainer: { border: '1px solid #dee2e6', display: 'inline-block', maxWidth: '100%' },
        heatmapImage: { display: 'block', maxWidth: '100%', height: 'auto' },
        cardSortResultContainer: { display: 'flex', flexDirection: 'column', gap: '15px', },
        cardSortResultCategory: { border: '1px solid #e0e0e0', borderRadius: '6px', backgroundColor: '#f9f9f9', padding: '15px', },
        cardSortCategoryTitle: { margin: '0 0 10px 0', fontSize: '1.1em', fontWeight: 'bold', color: '#444', borderBottom: '1px solid #eee', paddingBottom: '5px', },
        cardSortCardList: { listStyle: 'none', padding: 0, margin: 0, },
        cardSortCardItem: { display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontSize: '0.95em', borderBottom: '1px dotted #eee' },
        cardSortCardName: { color: '#333', },
        cardSortCardCount: { color: '#666', fontSize: '0.9em', whiteSpace: 'nowrap', marginLeft: '10px', },
        cardSortEmptyCategory: { fontSize: '0.9em', color: '#777', fontStyle: 'italic', padding: '10px 0', },
        loadingErrorText: { textAlign: 'center', padding: '40px', fontSize: '1.2em', color: '#dc3545' },
        backLink: { display: 'inline-block', marginBottom: '20px', color: '#0d6efd', textDecoration: 'none' },
        npsScoreContainer: { marginBottom: '20px' },
        npsScoreLabel: { fontSize: '1.2em', fontWeight: 'bold', display: 'block', marginBottom: '10px' },
        npsCombinedBar: { display: 'flex', height: '30px', width: '100%', borderRadius: '5px', overflow: 'hidden', border: '1px solid #ccc' },
        npsBarSegment: { height: '100%', transition: 'width 0.5s ease-in-out' },
        npsPromoterColor: { backgroundColor: '#28a745' },
        npsPassiveColor: { backgroundColor: '#ffc107' },
        npsDetractorColor: { backgroundColor: '#dc3545' },
        rankDistContainer: { display: 'flex', alignItems: 'flex-end', height: '40px', width: '100%', borderBottom: '1px solid #ccc', padding: '0 5px', boxSizing: 'border-box' },
        rankDistBar: { flex: 1, backgroundColor: '#a6d8a8', margin: '0 2%', transition: 'height 0.3s ease' },
        rankDistLabels: { display: 'flex', justifyContent: 'space-between', fontSize: '0.75em', color: '#6c757d', marginTop: '2px', padding: '0 5px' },
        writeInContainer: { marginTop: '15px', borderTop: '1px dashed #ccc', paddingTop: '10px' },
        writeInHeader: { fontSize: '0.95em', color: '#333', marginBottom: '5px', display: 'block' },
        writeInList: { listStyle: 'none', paddingLeft: '15px', maxHeight: '150px', overflowY: 'auto', fontSize: '0.9em' },
        writeInItem: { marginBottom: '3px' },
        writeInCount: { color: '#6c757d', marginLeft: '5px', fontSize: '0.9em' },
        exportLink: { display: 'inline-block', padding: '8px 15px', backgroundColor: '#198754', color: 'white', textDecoration: 'none', borderRadius: '5px', fontSize: '0.9em', fontWeight: 'bold', transition: 'background-color 0.2s ease', whiteSpace: 'nowrap', },
    };
