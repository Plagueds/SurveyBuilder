...
// ----- START OF COMPLETE MODIFIED FILE (vNext4.4 - Handle resumeToken from Email Link Query) -----
...
const PublicSurveyHandler = () => {
    ...
    useEffect(() => {
        ...
        console.log(`[PublicSurveyHandler] Mounted. Access Identifier: ${accessIdentifier}, Resume Token from Query: ${resumeTokenFromQuery}`);

        if (resumeTokenFromQuery && accessIdentifier) {
            console.log(`[PublicSurveyHandler] Resume token '${resumeTokenFromQuery}' found in query for collector '${accessIdentifier}'. Attempting direct resume.`);
            setViewMode('loading_action'); // Show loading while we fetch details for navigation
            // We need to get surveyId and collectorId to navigate correctly.
            // The accessPublicSurvey can give us this if successful (even if password protected, it might give IDs before full access)
            // Or, resumeSurveyWithCode could be adapted if it can resolve accessIdentifier to surveyId/collectorId first.
            // For now, let's assume accessPublicSurvey is the first step to get IDs.
            
            const attemptDirectResumeNavigation = async () => {
                try {
                    // Perform a quick check to get surveyId and collectorId
                    // No password sent here, as the token itself is the authorization for resume
                    ...
                    if (response && response.data && response.data.surveyId && response.data.collectorId) {
                        ...
                        console.log(`[PublicSurveyHandler] Navigating to SurveyTakingPage for email resume: /surveys/${surveyId}/c/${actualCollectorId}/${resumeTokenFromQuery}`);
                        // For email resume, we don't have partialResponse data yet in PublicSurveyHandler,
                        // SurveyTakingPage will fetch it using the resumeToken.
                        navigate(`/surveys/${surveyId}/c/${actualCollectorId}/${resumeTokenFromQuery}`, { 
                            ...
                            state: { 
                                ...
                                // isResumingWithCode: false, // Or a new flag like isResumingWithEmailToken
                            } 
                        });
                    } else {
                        console.error('[PublicSurveyHandler] Could not get survey/collector IDs for direct email resume.', response);
                        setError(response?.message || "Failed to process resume link. Survey details missing.");
                        setViewMode('error_page');
                    }
                } catch (err) {
                    console.error('[PublicSurveyHandler] Error during direct email resume navigation attempt:', err);
        ...
        // --- Standard initial check if not direct resuming from email query token ---
        if (!accessIdentifier) { /* ... same as vNext4.3 ... */ setError('No access identifier provided in the link.'); setViewMode('error_page'); toast.error('Invalid survey link: Missing identifier.'); return; }
        setViewMode('initial_check'); 
        const performInitialCheck = async () => { /* ... same as vNext4.3 ... */ try { console.log(`[PublicSurveyHandler] Performing initial check for: ${accessIdentifier}`); const response = await surveyApiFunctions.accessPublicSurvey(accessIdentifier, null); if (response && response.data) { setSurveyTitle(response.data.surveyTitle || ''); if (response.data.surveySettings && response.data.surveySettings.behaviorNavigation) { setSurveySettings(response.data.surveySettings.behaviorNavigation); console.log("[PublicSurveyHandler] Initial check - survey settings:", response.data.surveySettings.behaviorNavigation); } else { console.warn("[PublicSurveyHandler] Initial check - surveySettings.behaviorNavigation not found. Defaulting."); setSurveySettings({ saveAndContinueEnabled: true, saveAndContinueMethod: 'both' }); } if (response.success) { setInitialCheckRequiresPassword(false); } else if (response.requiresPassword) { setInitialCheckRequiresPassword(true); if (response.data.surveySettings && response.data.surveySettings.behaviorNavigation && !surveySettings) { setSurveySettings(response.data.surveySettings.behaviorNavigation); } } else { setError(response.message || "Could not retrieve initial survey information."); } } else { setError(response?.message || "Failed to get survey information during initial check."); setSurveySettings({ saveAndContinueEnabled: true, saveAndContinueMethod: 'both' }); } } catch (err) { const errData = err.response?.data || err; setSurveyTitle(errData?.surveyTitle || ''); if (errData?.requiresPassword) { setInitialCheckRequiresPassword(true); } else { setError(errData?.message || "Error during initial survey check."); } if (errData?.surveySettings?.behaviorNavigation) { setSurveySettings(errData.surveySettings.behaviorNavigation); } else { setSurveySettings({ saveAndContinueEnabled: true, saveAndContinueMethod: 'both' }); } console.error("[PublicSurveyHandler InitialCheck Catch]", err); } finally { setViewMode('choice'); } };
        performInitialCheck();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [accessIdentifier, location.search, navigate]); // Added location.search and navigate

    // ... (handleAttemptSurveyStart, handleResumeWithCode, handlePasswordSubmitForm are the same as vNext4.3) ...
    ...
    if (viewMode === 'initial_check' || (viewMode !== 'error_page' && !surveySettings && !location.search.includes('resumeToken'))) { 
        // Show loading if initial check is running OR if settings aren't loaded yet AND it's not a direct resume attempt from email
    ...
    // surveySettings might still be null if direct resume attempt is in progress, handle gracefully
    const saveAndContinueEnabledBySurvey = surveySettings?.saveAndContinueEnabled ?? false; // Default to false if settings not loaded
