// backend/server.js
// ----- START OF COMPLETE MODIFIED FILE (Robust CORS Update) -----
...
// --- START CORS CONFIGURATION (Robust Version) ---
const allowedOriginsFromEnv = [
    process.env.FRONTEND_URL, // Your deployed Netlify frontend (e.g., https://surveybuilder.netlify.app)
...
// This log line is crucial for debugging on Render. Check Render's startup logs for this.
console.log('[CORS] Normalized Allowed Origins on Startup:', normalizedAllowedOrigins);

const corsOptions = {
    origin: function (origin, callback) {
        // Normalize the incoming origin by removing a potential trailing slash
        const normalizedIncomingOrigin = origin ? origin.replace(/\/$/, '') : origin;

        // Allow requests with no origin (like mobile apps, Postman, curl)
        // or if the normalized origin is in our normalized whitelist
        if (!normalizedIncomingOrigin || normalizedAllowedOrigins.includes(normalizedIncomingOrigin)) {
            callback(null, true);
        } else {
            // This log will show what was received vs what's allowed if there's a mismatch during a request
...
// --- END CORS CONFIGURATION (Robust Version) ---
...
// ----- END OF COMPLETE MODIFIED FILE (Robust CORS Update) -----