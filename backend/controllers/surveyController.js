...
// ----- START OF COMPLETE MODIFIED FILE (vX.X+5 - All Functions Restored & savePartialResponse Corrected) -----
...
const generateConjointProfiles = (attributes) => { if (!attributes || attributes.length === 0) return []; return [];}; // Placeholder
const CSV_SEPARATOR = '; '; 
    ...
    try {
        ...
        if (forTaking === 'true') {
            ...
            if (resumeToken) {
                ...
                console.log(`[getSurveyById] Constructed answersMap:`, JSON.stringify(answersMap));
                console.log(`[getSurveyById] Constructed otherInputValuesMap:`, JSON.stringify(otherInputValuesMap));
        ...
        if (forTaking === 'true') {
            ...
            surveyResponseData.collectorSettings = {
                ...
                saveAndContinueMethod: collectorWebLink.saveAndContinueMethod || surveyBehaviorNav.saveAndContinueMethod || 'both',
                ...
                questionNumberingFormat: collectorWebLink.questionNumberingFormat || surveyBehaviorNav.questionNumberingFormat || '123',
                questionNumberingCustomPrefix: collectorWebLink.questionNumberingCustomPrefix || surveyBehaviorNav.questionNumberingCustomPrefix || '',
            };
        ...
        console.log(`[getSurveyById] Successfully fetched survey: ${surveyId}. Partial data present: ${!!surveyResponseData.partialResponse}, Answers in partial: ${JSON.stringify(surveyResponseData.partialResponse?.answers)}`);
        if (!res.headersSent) {
...
exports.updateSurvey = async (req, res) => {
    ...
    console.log(`[SurveyCtrl updateSurvey] ID: ${surveyId} - Received updates:`, JSON.stringify(updates, null, 2));
    if (!mongoose.Types.ObjectId.isValid(surveyId)) { return res.status(400).json({ success: false, message: 'Invalid Survey ID.' }); }
    if (!req.user || !req.user.id) { console.error('[updateSurvey] User ID not found. Auth middleware issue?'); return res.status(401).json({ success: false, message: 'User authentication failed.' }); }
    
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const survey = await Survey.findById(surveyId).session(session);
        if (!survey) { await session.abortTransaction(); session.endSession(); console.log(`[updateSurvey] Survey not found: ${surveyId}`); return res.status(404).json({ success: false, message: 'Survey not found.' }); }
        if (String(survey.createdBy) !== String(req.user.id) && req.user.role !== 'admin') { await session.abortTransaction(); session.endSession(); console.log(`[updateSurvey] Unauthorized attempt by user ${req.user.id} for survey ${surveyId}`); return res.status(403).json({ success: false, message: 'Not authorized to update this survey.' }); }
        
        console.log(`[SurveyCtrl updateSurvey] Survey Status BEFORE any local update: ${survey.status}`);
        if (updates.hasOwnProperty('status')) { console.log(`[SurveyCtrl updateSurvey] Update payload CONTAINS status: "${updates.status}"`); } 
        else { console.log(`[SurveyCtrl updateSurvey] Update payload DOES NOT CONTAIN status field.`); }
        ...
        for (const key of allowedTopLevelFields) {
            if (updates.hasOwnProperty(key)) {
                if (key === 'status') { console.log(`[SurveyCtrl updateSurvey] Attempting to update survey.${key} from "${survey[key]}" to "${updates[key]}"`); }
                survey[key] = updates[key];
            }
        }
        if (updates.settings && typeof updates.settings === 'object') {
            console.log(`[SurveyCtrl updateSurvey] Updating nested settings. Current survey.settings:`, JSON.stringify(survey.settings, null, 2));
            console.log(`[SurveyCtrl updateSurvey] Updates for settings:`, JSON.stringify(updates.settings, null, 2));
            survey.settings = survey.settings || {};
            const defaultBehaviorNav = { autoAdvance: false, questionNumberingEnabled: true, questionNumberingFormat: '123', questionNumberingCustomPrefix: '', saveAndContinueEnabled: false, saveAndContinueEmailLinkExpiryDays: 7, saveAndContinueMethod: 'email' };
            const defaultCustomVariables = [];
            for (const categoryKey in updates.settings) {
                if (Object.prototype.hasOwnProperty.call(updates.settings, categoryKey)) {
                    if (categoryKey === 'customVariables') {
                        survey.settings.customVariables = Array.isArray(updates.settings.customVariables) ? updates.settings.customVariables.filter(cv => cv.key && cv.key.trim() !== '') : defaultCustomVariables;
                        ...
                        if (categoryKey === 'behaviorNavigation' && survey.settings.behaviorNavigation) {
                            ...
                            if (updates.settings.behaviorNavigation.hasOwnProperty('saveAndContinueMethod') && !validMethods.includes(survey.settings.behaviorNavigation.saveAndContinueMethod)) {
            ...
            console.log(`[SurveyCtrl updateSurvey] Survey settings AFTER merge:`, JSON.stringify(survey.settings, null, 2));
        }
        if (updates.questions && Array.isArray(updates.questions)) {
            console.log(`[SurveyCtrl updateSurvey] Updating questions. Received ${updates.questions.length} questions in payload.`);
            survey.questions = updates.questions.map(q => q._id || q); // Store only IDs in survey.questions array
            const questionUpdatePromises = [];
            updates.questions.forEach((qData, index) => {
                const targetIndex = (qData && typeof qData.originalIndex === 'number') ? qData.originalIndex : index;
                if (qData && qData._id) {
                    console.log(`[SurveyCtrl updateSurvey] Scheduling update for Question ID: ${qData._id} to originalIndex: ${targetIndex}`);
                    questionUpdatePromises.push( Question.updateOne({ _id: qData._id }, { $set: { originalIndex: targetIndex } }).session(session) );
                } else {
                    console.warn(`[SurveyCtrl updateSurvey] Question data missing _id in questions array at index ${index}`, qData);
                }
            });
            if (questionUpdatePromises.length > 0) {
                await Promise.all(questionUpdatePromises);
                console.log(`[SurveyCtrl updateSurvey] Finished updating originalIndex for ${questionUpdatePromises.length} questions.`);
            }
        }
        survey.updatedAt = Date.now();
        console.log(`[SurveyCtrl updateSurvey] Survey Status AFTER applying all updates, BEFORE save: ${survey.status}`);
        console.log(`[SurveyCtrl updateSurvey] survey.isModified('status'): ${survey.isModified('status')}`);
        console.log(`[SurveyCtrl updateSurvey] survey.isModified('settings'): ${survey.isModified('settings')}`);
        console.log(`[SurveyCtrl updateSurvey] Full survey object BEFORE save:`, JSON.stringify(survey.toObject(), null, 2));
        const updatedSurvey = await survey.save({ session });
        console.log(`[SurveyCtrl updateSurvey] Survey Status AFTER save (from save result): ${updatedSurvey.status}`);
        ...
        console.log(`[updateSurvey] Survey ${surveyId} updated successfully. Final status: ${populatedSurvey.status}`);
        if (!res.headersSent) {
        ...
        session.endSession();
        console.error(`[SurveyCtrl updateSurvey] Error updating survey ${surveyId}. User: ${req.user?.id}. Error:`, error.stack);
        if (error.name === 'ValidationError') { if (!res.headersSent) return res.status(400).json({ success: false, message: 'Validation Error', details: error.errors }); }
        if (!res.headersSent) res.status(500).json({ success: false, message: 'Error updating survey on the server.' });
    }
};
...
exports.deleteSurvey = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(surveyId)) { return res.status(400).json({ success: false, message: 'Invalid Survey ID.' }); }
    if (!req.user || !req.user.id) { console.error('[deleteSurvey] User ID not found. Auth middleware issue?'); return res.status(401).json({ success: false, message: 'User authentication failed.' }); }
    
    const session = await mongoose.startSession();
    session.startTransaction();
    try {
        const survey = await Survey.findById(surveyId).select('createdBy').session(session);
        if (!survey) { await session.abortTransaction(); session.endSession(); console.log(`[deleteSurvey] Survey not found: ${surveyId}`); return res.status(404).json({ success: false, message: 'Survey not found.' }); }
        if (String(survey.createdBy) !== String(req.user.id) && req.user.role !== 'admin') { await session.abortTransaction(); session.endSession(); console.log(`[deleteSurvey] Unauthorized attempt by user ${req.user.id} for survey ${surveyId}`); return res.status(403).json({ success: false, message: 'Not authorized to delete this survey.' }); }
...
exports.savePartialResponse = async (req, res) => {
    ...
    const { 
        collectorId, 
        respondentEmail, 
        answers: currentAnswers, // Corrected destructuring
        otherInputValues, 
        currentVisibleIndex, 
        visitedPath, 
        sessionId, 
...
exports.getSurveyResults = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(surveyId)) { return res.status(400).json({ success: false, message: 'Invalid Survey ID.' }); }
    if (!req.user || !req.user.id) { console.error('[getSurveyResults] User ID not found in request. Auth middleware issue?'); return res.status(401).json({ success: false, message: 'User authentication failed.' }); }
    try {
        ...
        if (!survey) { if (!res.headersSent) return res.status(404).json({ success: false, message: 'Survey not found.' }); return; }
        ...
        if (responseHeaders.length === 0) {
            if (!res.headersSent) return res.status(200).json({ success: true, surveyTitle: survey.title, questions: survey.questions, summary: { totalResponses: 0 }, data: [] });
            return;
        }
        ...
        if (!res.headersSent) {
            res.status(200).json({ success: true, surveyTitle: survey.title, questions: survey.questions, summary, data: reconstructedResponses });
        }
    } catch (error) {
...
exports.exportSurveyResults = async (req, res) => {
    ...
    try {
        ...
        if (responseHeaders.length === 0 && format.toLowerCase() === 'csv') {
            ...
            return res.status(200).send("No responses to export."); // Note: res.send() will set headersSent
        } else if (responseHeaders.length === 0) {
            if (!res.headersSent) return res.status(200).json({ success: true, surveyTitle: survey.title, totalResponses: 0, data: [] });
            return;
        }
        ...
        if (format.toLowerCase() === 'csv') {
            const questionsMap = new Map(survey.questions.map(q => [q._id.toString(), q]));
            const csvFields = [
                { label: 'Response ID (Internal)', value: '_id' }, // Changed from 'Response ID (Session)'
                ...
                // { label: 'Duration (seconds)', value: 'durationSeconds'}, // Assuming durationSeconds is not directly on header
            ];
            survey.questions.sort((a,b) => a.originalIndex - b.originalIndex).forEach(q => {
                csvFields.push({
                    label: q.text || `Question ${q.originalIndex + 1}`,
                    value: (row) => {
                        const answerObj = row.answers?.[q._id.toString()]; // answerValue is directly the value
            ...
            res.status(200).send(csv); // Note: res.send() will set headersSent
        } else {
            if (!res.headersSent) {
                res.status(200).json({ success: true, surveyTitle: survey.title, totalResponses: reconstructedResponses.length, data: reconstructedResponses });
            }
        }
    } catch (error) {
...
// ----- END OF COMPLETE MODIFIED FILE (vX.X+5 - All Functions Restored & savePartialResponse Corrected) -----